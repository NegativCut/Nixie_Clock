...
const int cs5 = 6;
const int cs6 = 5;
int brightness = 55; // Default, loaded from EEPROM
...
// RGB LED values (loaded from EEPROM or defaults)
int led_red = 127;
int led_green = 0;
int led_blue = 127;
...
// Timing constants
const long DATE_DISPLAY_INTERVAL = 55000; // Time display duration before showing date
const long DATE_SHOW_DURATION = 5000;     // How long to show date
const int FRAME_INTERVAL = EXIXE_ANIMATION_FRAME_DURATION_MS; // 33ms per frame
const int LED_REFRESH_INTERVAL = 1000;    // Refresh LEDs every 1s
const int ENCODER_POLL_INTERVAL = 5;      // Poll encoder every 5ms (reduced from 2ms)
const int ENCODER_DEBOUNCE_DELAY = 20;    // Debounce delay increased to 20ms (from 10ms)
const int SHORT_PRESS_THRESHOLD = 200;    // Short press under 200ms
const int LONG_PRESS_THRESHOLD = 1000;    // Long press over 1000ms
const int BLINK_INTERVAL = 500;           // Blink rate for set mode
const int CROSSFADE_DURATION = 10;        // Reduced to 10 frames (330ms)
...
const int ANTI_POISON_CYCLE_DURATION = 100; // Duration per digit in ms

unsigned long previousMillis = 0;
unsigned long lastFrameTime = 0;
unsigned long lastLedRefresh = 0;
unsigned long lastAntiPoisonCycle = 0;
unsigned long setModeEntryTime = 0xFFFFFFFF; // Initialize to max value to prevent false timeout
...
void setup() {
  ...
  if (needsTimeSet) {
    ...
    // Add offset for compile + upload time
...
void loop() {
  ...
  if (currentMode != NORMAL) {
    if (currentMillis - setModeEntryTime >= SET_MODE_TIMEOUT) {
      ...
      currentMode = NORMAL;
      setModeEntryTime = 0; // Reset timeout timer
    }
  }

  // Anti-cathode poisoning cycle
  if (currentMode == NORMAL && !isDateMode && 
      currentMillis - lastAntiPoisonCycle >= ANTI_POISON_INTERVAL) {
...
void loadSettingsFromEEPROM() {
  ...
  if (EEPROM.read(EEPROM_MAGIC_ADDR) == EEPROM_MAGIC_VALUE) {
    ...
    led_red = constrain(led_red, 0, 255);
    led_green = constrain(led_green, 0, 255);
    led_blue = constrain(led_blue, 0, 255);
  } else {
...
void runAntiPoisonCycle() {
  isAntiPoisonRunning = true;
  exixe* tubes[6] = {&my_tube1, &my_tube2, &my_tube3, &my_tube4, &my_tube5, &my_tube6};
  
  // Cycle through all digits 0-9 on all tubes
  for (int digit = 0; digit <= 9; digit++) {
    for (int i = 0; i < 6; i++) {
      tubes[i]->show_digit(digit, brightness, overdrive);
    }
    delay(ANTI_POISON_CYCLE_DURATION);
  }
...
void flashErrorPattern() {
  ...
  for (int i = 0; i < 6; i++) {
    tubes[i]->spi_init();
    tubes[i]->set_led(255, 0, 0); // Red LEDs
  }
...
void flashWarningPattern() {
  ...
  for (int i = 0; i < 6; i++) {
    tubes[i]->set_led(255, 127, 0); // Orange LEDs
  }
...
void flashSetConfirmation() {
  ...
  for (int i = 0; i < 6; i++) {
    tubes[i]->set_led(0, 255, 0); // Green LEDs
  }
  ...
  if (currentSwitchState && !switchPressed) {
    ...
    if (pressDuration < SHORT_PRESS_THRESHOLD) { // Short press
      if (currentMode == NORMAL) {
        ...
        // After setting seconds, save time and exit immediately
        saveTimeOnly();
        currentMode = NORMAL;
        setModeEntryTime = 0; // Reset timeout timer
...
void readEncoder() {
  ...
  encoderState = (encoderState << 2) | (MSB << 1) | LSB;
  
  // Only process if in a set mode
  if (currentMode != NORMAL) {
    if (millis() - lastDebounceTime >= ENCODER_DEBOUNCE_DELAY) {
      // Look up direction from state table
      int8_t direction = ENCODER_TABLE[encoderState & 0x0F];
...
void updateBrightnessDisplay() {
  // Blank first 4 digits, show brightness in last 2
  target_digits[0] = 0;
  target_digits[1] = 0;
  target_digits[2] = 0;
  target_digits[3] = 0;
  target_digits[4] = brightness / 10;
...
void adjustSetValue() {
  ...
  if (delta != 0) {
    ...
    if (currentMode == SET_BRIGHTNESS) {
      ...
      led_red = constrain(led_red + step, 0, 255);
      updateLEDColorDisplay();
    } else if (currentMode == SET_LED_GREEN) {
      led_green = constrain(led_green + step, 0, 255);
      updateLEDColorDisplay();
    } else if (currentMode == SET_LED_BLUE) {
      led_blue = constrain(led_blue + step, 0, 255);
      updateLEDColorDisplay();
      ...
      switch (currentMode) {
        ...
        case SET_DAY: {
          ...
          setValues.day = constrain(setValues.day + step, 1, maxDays);
          break;
        }
        case SET_MONTH:
          setValues.month = constrain(setValues.month + step, 1, 12);
...
void updateTubes(unsigned long currentMillis) {
  static exixe* tubes[6] = {&my_tube1, &my_tube2, &my_tube3, &my_tube4, &my_tube5, &my_tube6};

  // Don't update during anti-poison cycle
  if (isAntiPoisonRunning) return;

  // Update blink state for set modes
  if (currentMode != NORMAL && currentMode != SET_BRIGHTNESS && 
      currentMode != SET_LED_RED && currentMode != SET_LED_GREEN && 
      currentMode != SET_LED_BLUE && currentMillis - lastBlinkTime >= BLINK_INTERVAL) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    // Determine which digits should blink based on mode
    ...
    // Handle brightness mode blanking
    if (currentMode == SET_BRIGHTNESS && i < 4) {
      tubes[i]->clear();
      prev_digits[i] = -1;
      continue;
    }

    // Handle LED color mode blanking
    if ((currentMode == SET_LED_RED || currentMode == SET_LED_GREEN || currentMode == SET_LED_BLUE) && i < 3) {
      tubes[i]->clear();
      prev_digits[i] = -1;
      continue;
    }

    // Initialize crossfade only when digit changes and in normal mode
    if (prev_digits[i] != target_digits[i] && currentMode == NORMAL) {
      tubes[i]->crossfade_init(target_digits[i], CROSSFADE_DURATION, brightness, overdrive);
      prev_digits[i] = target_digits[i];
    }

    // Handle blinking in set modes (instant digit change, no crossfade)
    if (currentMode != NORMAL && currentMode != SET_BRIGHTNESS && 
        currentMode != SET_LED_RED && currentMode != SET_LED_GREEN && 
        currentMode != SET_LED_BLUE && shouldBlink) {
      if (blinkState) {
      ...
      prev_digits[i] = target_digits[i];
    } else if (currentMode != NORMAL && currentMode != SET_BRIGHTNESS && 
               currentMode != SET_LED_RED && currentMode != SET_LED_GREEN && 
               currentMode != SET_LED_BLUE) {
      // Non-blinking digits in set mode - show immediately
      tubes[i]->show_digit(target_digits[i], brightness, overdrive);
      prev_digits[i] = target_digits[i];
    } else if (currentMode == NORMAL) {
      // Normal mode - run crossfade animation
      tubes[i]->crossfade_run();
    } else if (currentMode == SET_BRIGHTNESS && i >= 4) {
      // Brightness mode - show last 2 digits immediately
      tubes[i]->show_digit(target_digits[i], brightness, overdrive);
      prev_digits[i] = target_digits[i];
    } else if ((currentMode == SET_LED_RED || currentMode == SET_LED_GREEN || currentMode == SET_LED_BLUE) && i >= 3) {
      // LED color mode - show last 3 digits immediately
      tubes[i]->show_digit(target_digits[i], brightness, overdrive);
      prev_digits[i] = target_digits[i];
    }
  }
